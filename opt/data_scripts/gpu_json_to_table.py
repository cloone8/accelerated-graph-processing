...
def print_table(graphdata, alg, graphfilter=""):
    print("\subsubsection{" + alg + "}")
    print("""
\\begin{table}[H]
\\begin{tabular}{lllllll}
\hline
\multicolumn{1}{|l|}{Graph}  &                            & CUDA                             & \multicolumn{1}{l|}{}       &                            & OpenACC                          & \multicolumn{1}{l|}{}       \\\ \hline
\multicolumn{1}{l|}{}        & \multicolumn{1}{l|}{Total} & \multicolumn{1}{l|}{Main loop} & \multicolumn{1}{l|}{Memory} & \multicolumn{1}{l|}{Total} & \multicolumn{1}{l|}{Main loop} & \multicolumn{1}{l|}{Memory} \\\ \hline"""
    ...
    for graph in toprocess:
        cuda_total = "\\begin{tabular}[c]{@{}l@{}}" + str(round(graphdata[graph][alg.upper() + "_CUDA"]["avg_total"], roundto)) + "\\\ $ \\pm  $ " + str(round(graphdata[graph][alg.upper() + "_CUDA"]["std_total"], roundto)) + "\end{tabular}"
        cuda_comp = "\\begin{tabular}[c]{@{}l@{}}" + str(round(graphdata[graph][alg.upper() + "_CUDA"]["avg_computation"], roundto)) + "\\\ $ \\pm  $ " + str(round(graphdata[graph][alg.upper() + "_CUDA"]["std_computation"], roundto)) + "\end{tabular}"
        cuda_mem = "\\begin{tabular}[c]{@{}l@{}}" + str(round(graphdata[graph][alg.upper() + "_CUDA"]["avg_memory"], roundto)) + "\\\ $ \\pm  $ " + str(round(graphdata[graph][alg.upper() + "_CUDA"]["std_memory"], roundto)) + "\end{tabular}"
